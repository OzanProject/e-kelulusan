...
<body>
    ...
    <div class="header-container">
        @if($data['kop_base64'])
            <img src="{{ $data['kop_base64'] }}" class="kop-surat">
        @else
    ...
    <div class="content">
        ...
        <div class="subtitle">Nomor: {{ $data['skl_number'] }}</div>
        ...
        <table class="table-data">
            <tr>
                <td>Nama Lengkap</td>
                <td>:</td>
                <td><strong>{{ $data['student']->nama_lengkap }}</strong></td>
            </tr>
            <tr>
                <td>NISN</td>
                <td>:</td>
            ...
            <tr>
                <td>Nomor Peserta (NIS)</td>
                <td>:</td>
                <td>{{ $data['student']->nomor_peserta }}</td>
            </tr>
        </table>

        <div class="status-box">
            @if($data['student']->status_kelulusan == 'lulus')
                LULUS
            @elseif($data['student']->status_kelulusan == 'tidak_lulus')
                TIDAK LULUS
            @else
                DITUNDA
            @endif
        ...
        <table class="table-nilai">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $data['student']->nilai_ujian[$s->kode] ?? '-' }}</td>
                </tr>
                @endforeach
            </tbody>
            <tfoot>
                <tr style="font-weight: bold;">
                    ...
                    <td>{{ $data['student']->nilai_ujian['TTL'] ?? '-' }}</td>
                </tr>
            </tfoot>
        </table>
        ...
        <div class="signature-container">
            <div>{{ $setting->school_address_city ?? 'Kota' }}, {{ $data['skl_date'] }}</div>
            <div>Kepala Sekolah,</div>
            
            <div style="height: 85px; position: relative;">
                @if($setting->signature_type == 'qr')
                    <img src="{{ $data['qrcode_base64'] }}" class="qr-code">
                @else
                    @if($data['stamp_base64'])
                        <img src="{{ $data['stamp_base64'] }}" class="stamp-img">
                    @endif
                    @if($data['signature_base64'])
                        <img src="{{ $data['signature_base64'] }}" class="manual-signature">
                    @endif
                @endif
            </div>
